    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if ((currentTime - messageTime) / 60 > 5) {
                return reject('Time elapsed between when the message was sent and the current time must be less that 5 minutes.');
            }

            const verifyResult = bitcoinMessage.verify(message, address, signature);
            if (!verifyResult) {
                return reject('No valid signature.')
            }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            self.chain.forEach(block => {
               const decodedBody = block.getBData();
               if (decodedBody) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            self.chain.forEach((block, index) => {
                ...
                if (block.height > 0) { // Not Genesis Block
                    ...
                    const blockHash = self.chain[index - 1].hash;
                    if (previousBlockHash !== blockHash) {
                        errorLog.push(`Chain is broken for Block: ${block.height}`);
                    }
                }
            });
            ...
            Promise.all(promises).then(validationResults => {
                validationResults.forEach((isValid, index) => {
                    if (!isValid) {
                        errorLog.push(`Block: ${index} hash is not valid`);
                    }
                })
            })
